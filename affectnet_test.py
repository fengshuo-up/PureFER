...
def parse_args():
    ...
    parser.add_argument('--aff_path', type=str, default='AfectNet dataset path', help='AfectNet dataset path.')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size.')
    parser.add_argument('--workers', default=8, type=int, help='Number of data loading workers.')
    parser.add_argument('--num_head', type=int, default=2, help='Number of attention head.')
    parser.add_argument('--num_class', type=int, default=8, help='Number of class.')   #7 or 8
...
def run_test():
    ...
    model = DDCMNet(num_class=args.num_class, num_head=args.num_head, pretrained=False)
    ...
    if args.num_class == 7: 
        ...
        plot_confusion_matrix(matrix, classes=class7_names, normalize=True,
                              title= 'Affectnet-7  Confusion Matrix (acc: %0.2f%%)' %(acc*100),
                              fontsize=10)
 		
        # plt.savefig(os.path.join('checkpoints_ver2.0', "affectnet7"+"_acc"+str(acc)+".png"))
        plt.savefig(os.path.join('checkpoints_ver2.0', f"affectnet7_acc{acc}.svg"),
                    format='svg',
                    bbox_inches='tight',
                    dpi=500)
        plt.close()
        ...
        plot_confusion_matrix(matrix, classes=class8_names, normalize=True,
                              title= 'AffectNet-8 Confusion Matrix (acc: %0.2f%%)' %(acc*100),
                              fontsize=10)

        plt.savefig(os.path.join('checkpoints_ver2.0', f"affectnet8_acc{acc}.svg"),
                    format='svg',
                    bbox_inches='tight',
                    dpi=500)
        plt.close()
